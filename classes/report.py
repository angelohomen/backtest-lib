...
class Report():
    ...
    def plot_report(self):
        ...
        # HOUR
        aux = self.__result_df.drop(['TradeType','TradeSide'],axis=1).groupby(by=self.__result_df['EntryTime'].dt.hour).mean()

        if len(aux.index) > 1: 
            cond = [(aux['TradeResult']>=0.0), (aux['TradeResult']<0.0)]
            esc = ['High', 'Low']
            aux['Color'] = np.select(cond, esc, default=None)
            order = aux.index.sort_values().unique().values
            axHour.set_title('Net Balance per hour ')
            sns.barplot(x=aux[aux['TradeResult']>0].index, y='TradeResult',data=aux[aux['TradeResult']>0], order=order,            
                        color='#6DC75E', ax=axHour)
            sns.barplot(x=aux[aux['TradeResult']<0].index, y='TradeResult',data=aux[aux['TradeResult']<0], order=order,            
                        color='#D6675A', ax=axHour)

        # WEEK DAYS
        aux = self.__result_df.drop(['TradeType','TradeSide'],axis=1).groupby(by=self.__result_df['EntryTime'].dt.day_name()).mean()
        ...
        #MONTH
        aux = self.__result_df.drop(['TradeType','TradeSide'],axis=1).groupby(by=self.__result_df['EntryTime'].dt.month_name()).mean()
